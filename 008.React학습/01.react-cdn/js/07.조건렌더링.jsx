...
function Developer({ isNormal, isrc, ialt, itit }) {
  ...
  // 위의 if문에 들어가면 그안의 return 문 때문에
  // 아랫쪽의 return문은 실행되지 않는ㄷ ㅏ!
  // else 문 불필요
...
// 컴포넌트 호출하기 1 : 개발자 찍기
ReactDOM.render(
  <Developer isNormal={false} isrc={devImg[0]} ialt="개발자공유" itit="프론트앤드 개발자 공유입니다" />,
  root[0]
);
...
// 전달할 음식리스트 배열변수
const foods = ["스파게티", "짜파게티", "냉면", "짜장면", "마라탕", "탕수육"];
...
function MakeList({ foodName, MovieInfo }) {
  ...
  return (
    <li>
      {
        //음식데이터가 들어온 경우 출력
        // 만약 데이터가 안들어오면 undifined이므로
        // false처리됨! 할당되면 true처리됨!
