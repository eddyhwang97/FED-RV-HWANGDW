...
// 2. 패션 영역 구현하기
// 2-1. 패션영역용 데이터 수집하기
// filter() 메서드는 조건이 맞을때 true 리턴
// ->결과 : 필터링된 배열값
const fashionData = itemData
  .filter((v) => {
    if (v.mainCat == "fashion") return true;
  })
  // 2-2. 패션영역 데이터 정렬하기 : idx 오름차순
  // fashionData.sort((a,b)=>a,idx==b.idx?0:a.idx<b.idx?-1:1)
  .sort((a, b) => (a.idx == b.idx ? 0 : a.idx < b.idx ? -1 : 1));

console.log("패션데이터", fashionData);
...
const todayData = itemData
  ...
  // 3-3. 투데이영역 데이터 정렬하기 : idx 오름차순
  // todayData.sort((a,b)=>a,idx==b.idx?0:a.idx<b.idx?-1:1)
  .sort((a, b) => (a.idx == b.idx ? 0 : a.idx < b.idx ? -1 : 1));

console.log("투데이데이터", todayData);
