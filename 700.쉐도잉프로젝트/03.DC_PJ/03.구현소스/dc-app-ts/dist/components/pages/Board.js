...
function Board() {
    ...
    const baseData = JSON.parse((_a = localStorage.getItem("board-data")) !== null && _a !== void 0 ? _a : "[]");
    // [ ★★ 후크 상태관리 변수구역 ★★ ] //////
    // [1] 기능모드 /////
    ...
    if (keyword.kw !== "") {
        finalData = baseData
            ...
            // 여기부터 검색어로 리스트 만들기
            .filter((v) => {
            if (v[keyword.cta].toLowerCase().indexOf(keyword.kw.toLowerCase()) !== -1)
                return true;
        }); ////// filter ////////
    } ///// if : 검색어가 있는 경우 /////////
    // [2] 검색어가 없는 경우 ////////
    ...
    return (<>
      {
        ...
        mode === "L" && (<List selData={selData} // 선택 리스트 배열데이터
      ...
      {
        // [2] 보기모드 출력하기 : mode -> "R" ///
        mode === "R" && (<Read setMode={setMode} // 모드 상태변수 setter
         selRecord={selRecord} // 선택데이터 참조변수
        />)}

      {
        // [3] 쓰기모드 출력하기 : mode -> "W" ///
        mode === "W" && (<Write setMode={setMode} // 모드 상태변수 setter
         totalCount={totalCount} // 전체 개수 참조변수
         setPageNum={setPageNum} // 리스트 페이지번호 setter
         pgPgNum={pgPgNum} // 페이징의 페이징 번호
         initVariables={initVariables} // 변수초기화함수
      ...
      {
        // [4] 수정모드 출력하기 : mode -> "M" ///
        mode === "M" && (<Modify setMode={setMode} // 모드 상태변수 setter
         selRecord={selRecord} // 선택데이터 참조변수
         totalCount={totalCount} // 전체 개수 참조변수
         setPageNum={setPageNum} // 리스트 페이지번호 setter
         pgPgNum={pgPgNum} // 페이징의 페이징 번호
