...
function Read({ setMode, selRecord }) {
    ...
    // 2. 세션스 글번호 저장하기 ////
    // (1) 세션스 파싱하여 변수할당 ///
    let rec = JSON.parse((_a = sessionStorage.getItem("bd-rec")) !== null && _a !== void 0 ? _a : "[]");
    ...
    if (!isRec) {
        // (1) 게시판 로컬스 데이터 파싱
        let bdData = JSON.parse((_b = localStorage.getItem("board-data")) !== null && _b !== void 0 ? _b : "[]");
    ...
    const saveComment = () => {
        ...
        let comDt = localStorage.getItem("comment-data")
            ? JSON.parse((_a = localStorage.getItem("comment-data")) !== null && _a !== void 0 ? _a : "[]")
            : [];
        console.log("코멘트 저장해~!", comDt.map((v) => v.idx));
    ...
    const makeCommentData = () => {
        ...
        if (localStorage.getItem("comment-data")) {
            let temp = JSON.parse((_a = localStorage.getItem("comment-data")) !== null && _a !== void 0 ? _a : "[]");
            temp = temp
    ...
    const deleteComment = (idx) => {
        ...
        let comDt = JSON.parse((_a = localStorage.getItem("comment-data")) !== null && _a !== void 0 ? _a : "[]");
    ...
    const modifyComment = (idx) => {
        ...
        setIsEditing(idx);
        // (2) idx값이 동일한 코멘트를 선택
        const selData = (_a = commentData.find((v) => v.idx === idx)) !== null && _a !== void 0 ? _a : { cont: "" };
    ...
    const saveModifiedComment = (idx) => {
        ...
        comDt = comDt.map((v) => v.idx === idx ? Object.assign(Object.assign({}, v), { cont: editedContent }) : v);
        // (3) 로컬스에 다시 저장!
        localStorage.setItem("comment-data", JSON.stringify(comDt));
    ...
    return (<main className="cont">
      ...
      <table className="dtbl btngrp">
        ...
        // 수정버튼 보이기
        myCon.loginSts && myCon.loginSts.uid === selData.uid && (<button onClick={() => {
                // 수정모드로 변경하기
                setMode("M");
            }}>
                    Modify
      ...
      {/* 코멘트 입력 박스 */}
      {
        ...
        myCon.loginSts && (<table className="dtblview">
            <tbody>
              <tr>
                <td>Comments</td>
                <td>
      ...
      {
        // 코멘트가 있으면 출력 (상태변수로체크!)
        commentData && (<table className="dtblview">
            <tbody>
              {
            // 코멘트 테이터 만큼 반복생성하기
            commentData.map((v, i) => (<tr key={i}>
                    {/* (1) 코멘트 쓴이 이름 */}
                ...
                */}
                    <td>
                      <textarea className="comment-view-box" 
            // 내용에 따른 높이값 정보를 참조변수에 노출
            // 자기자신을 참조변수 textareaRef에 할당!
            ref={(el) => {
                    textareaRef.current[i] = el;
                }} value={isEditing === v.idx // 수정해당이면
