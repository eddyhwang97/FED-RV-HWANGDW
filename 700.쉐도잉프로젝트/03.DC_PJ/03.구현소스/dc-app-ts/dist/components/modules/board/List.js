...
// 제이쿼리 불러오기 ///
import $ from "jquery";
function List({ selData, // 선택된 배열데이터 전달
...
initVariables, // 변수초기화함수
    ...
    ***********************************/
    const pagingCode = () => {
        ...
        if (pgPgNum.current !== 1)
            hcode.push(<Fragment key="-1">
          {/* 처음 페이징으로 이동하기 */}
          <a href="#" title="First Paging Section" onClick={(e) => {
                    e.preventDefault();
          ...
          {/* 이전 페이징으로 이동하기 */}
          <a href="#" title="Previous Paging Section" onClick={(e) => {
                    e.preventDefault();
                    // (1) 페이징의 페이징번호 감소
                    pgPgNum.current--;
        ...
        for (let i = initNum; i < limitNum; i++) {
            ...
            hcode.push(<Fragment key={i}>
          {
                // 현재 페이지와 일치되는번호는
                // a태그가 아닌 b태그로 표시!
                i + 1 === pageNum ? (<b>{i + 1}</b>) : (<a href="#" onClick={(e) => {
                        e.preventDefault();
                        // 페이지번호 업데이트하기
                        setPageNum(i + 1);
                    }}>
        ...
        if (pgPgNum.current !== pgPgLimit)
            hcode.push(<Fragment key="-2">
          {/* 다음 페이징 이동하기 */}
          <a href="#" title="Next Paging Section" onClick={(e) => {
                    e.preventDefault();
                    // (1) 페이징의 페이징번호 증가
                    pgPgNum.current++;
          ...
          {/* 맨끝 페이징 이동하기 */}
          <a href="#" title="Last Paging Section" onClick={(e) => {
                    e.preventDefault();
    ...
    *******************************************/
    ...
    (_a = localStorage.getItem("memory-data")) !== null && _a !== void 0 ? _a : "");
    // 1. memory : 검색어 저장변수
    // -> 값은 *로 구분자를 사용한 문자열
    ...
    defaultValue={keyword.cta}>
        ...
        <select name="sel" id="sel" className="sel" value={order} onChange={(e) => {
            // 정렬값 반대로 변경하기
            setOrder(order * -1);
            // 변경시 변경한 선택값 반영하기
        ...
        <input id="stxt" type="text" maxLength={50} defaultValue={keyword.kw} onKeyUp={(e) => {
            // 엔터를 친 경우 ///
            if (e.key === "Enter")
        ...
        {/* 리듀서를 이용한 검색어 표시버튼 */}
        <button style={{ position: "relative" }} onMouseLeave={(e) => {
            // 마우스가 벗어나면 검색레코드 숨기기
        ...
        {/* 정렬기준 선택박스 */}
        <select name="sort_cta" id="sort_cta" className="sort_cta" style={{
            float: "right",
            translate: "0 5px",
        }} value={sortCta} onChange={(e) => {
            // 정렬기준 변경하기
      ...
      <table className="dtbl" id="board">
        ...
        totalCount.current > 0 ? (selData.map((v, i) => (<tr key={i}>
                  <td>
                    {
            // 페이징 시작번호 더하기
            // -> 자동순번 + (단위수 * (페이지번호-1))
