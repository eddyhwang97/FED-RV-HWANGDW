...
function showFruit() {
  ...
  // (2)과일별 기능 분기하기
  // (2-1) '과일주세요~!' 버튼 :  하단박스에 과일 이미지 출력
  if (btxt === "과일주세요~!") {
    // 출력대상 : .cont -> cont변수
    cont.innerHTML = `
    <ul>
    ${fruit
      // .map((v) => {return `<li>zz</li>`}) 중괄호 쓸경우 return 키워드 사용해야함
      .map(
        (v) =>
          `<li style ="background: url(./addimg/${frObj[v]}.png) no-repeat center/cover">${v}</li>`
      )
      .join("")}
    </ul> 
    `;
