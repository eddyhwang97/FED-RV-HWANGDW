...
//  3. 함수만들기
    function changeImage() {
        // 3-1 함수호출확인
        console.log("changeImage 함수 호출",this);
        // this키워드는 함수를 호출한 자기 자신!

        // 2. 호출한 요소(버튼)의 텍스트 읽기
        var bTxt = this.innerText;
        console.log("버튼텍스트:",bTxt);

        // 3. 이미지 경로if문으로 분기하여 만들기
        var isrc = "./images/ala1.jpg";
        if(bTxt === "포스터"){
        ...
        // 6. 이미지 변경하기
        scene.src = isrc;
        // 7. 클래스 off제거하기 : 왼쪽 바깥 투명
        scene.classList.remove('off');
        // 8. 클래스 on넣기 : 오른쪽 바깥에 투명
        scene.classList.add('on');
        },500);

        // 1초 후 코드실행 : 위의 타임아웃보다 0.5초 후 ///
        setTimeout(function(){
        // 9. 클래스 on 제거하기 : 원래 위치로 등장
