...
// 단, 제이슨 파일을 불러오려면 import문법을 써야하는데
// 모듈문법을 사용해야 쓸 수 있다.
...
// 1. 다국어 요구사항
// - 언어박스에서 언어를 변경하면 코드에 맞게 다국어 데이터를 제이슨 파일에서 읽어와 본 페이지의 해당 요소를 변경한다!
...
// (3) 하단주소 : #info address
const address = myFn.qs("#info address");

// console.log("대상요소 : ",selBox, gnbList, mainImg, address);

// 3. 이벤트설정
myFn.addEvt(selBox, 'change', setLang);

// 4. 함수만들기 (건포도식빵 에서 건포도만 뽑아먹듯 js에서 함수를 먼저 골라 읽는다)
function setLang(){
    // this는 호출한 선택박스 요소
    // this.value 는 선택된 option 요소 value값
    ...
    // 3. 메인이미지 src변경하기 
    // 각 언어별 이미지명이 선택박스의 value값으로 만들어져 있으므로 
    // 이미지명을 optval로 넣어준다
