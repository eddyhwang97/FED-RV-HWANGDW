...
const arr1 = new Array(
  ...
  function(){
    // this 는 누구? 호출한 요소 자신
    // alert("김태리 멋찜!");
    // 1. 호출한 요소 박스에  김태리 사진을 넣고 
    this.innerHTML += `
    <img src="https://img.vogue.co.kr/vogue/2021/02/style_6020aef125358-743x930.jpg" alt="김태리이미지" 
    style="
    position: absolute;
    top: 0;
    left: 0;
    ...
    this.onmousemove = (헐) => {
      ...
      // 곧 그것이 이벤트 전달변수가 됨!
      // 그요소에서 발생하는 이벤트를 객체로 가지고 있음
      // 대체해서 evevt라고 직접 전제 이벤트 객체를 쓸 수 있음
      // 이벤트 객체하위 pageX, pageY는 최상단 , 최왼쪽으로부터 
      // 마우스 커서의 위치를 x,y축으로 단위없는 px값을 리턴한다.
      // 이값은 사이트 전체를 이동하는 어떤 요소를 구현할 때 많이 사용한다.

      // console.log("pageX: " , 헐.pageX);
      // console.log("pageY: " , 헐.pageY);

      // 여기서는 본 박스 안에서만 위치를 알면 되므로 
      // -> offsetX, offsetY -> 해당 부모박스 요소로부터 위치를 리탄함

      // console.log("offsetX: ", 헐.pageX);
      // console.log("offsetY: ", 헐.pageY);
      console.log("offsetX: ", 헐.pageX,"offsetY: ", 헐.pageY);
...
// typeof(변수) / typeof 변수 -> 테이터형 출력
// 배열의 데이터형을 찍으면 object라고 나옴
// 이것은 배열도 객체이기 때문!
// 배열인지 검사하는 방법 Array.isArray(변수)
// ->배열야부를 알아내는 메서드는 중요함
...
// 배열끝에 컴마는 원래는 쓰면 에러나지만
// 최신 브라우저에서 에러안나게 처리됨
// 요즘은 추가 배열 데이터를 위해 마지막 콤마를
// 쓰는것이 일반적임!(프리티어도 마지막콤마 넣어줌)
