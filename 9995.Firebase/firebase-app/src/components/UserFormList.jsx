...
const UserFormList = () => {
  ...
  const getUserList = async (page, startDoc) => {
    // page : 현재 페이지, startDoc : 이전 페이지 데이터 배열
    // -> 이전페이지 데이터 배열이 왜 필요하지?
    // 이유는 startAfter라고 하는 함수에서 시작 문서가
    // 이전페이지의 마지막 문서를 넣어야 하므로 배열 데이터를
    // 이전페이지것을 가져와야 한다.
    ...
    if (page === 1) {
      ...
      q = query(setCollection, orderBy(sortField, sortOrder), startAfter(startDoc[page - 2]), limit(PAGE_SIZE));
      // 왜 2를 빼지? 현재 페이지 번호 보다 배열번호는 1작기 때문에 2페이지의 이전페이지는 1이 아니라 0임
      // 이런원리로 2를 빼야 배열 순번이 일치함
      // 이 배열 순번은 startDoc배열의 순번을 읽어오는 것임
      // 2페이지->startDoc[0]
      // 3페이지->startDoc[1]
      // 4페이지->startDoc[2]
      // orderBy(정렬필드,정렬순서) 메서드를 사용하여
      // 정렬 필드와 순서를 설정하고
      // query() 메서드를 사용하여 쿼리를 생성함
      console.log(startDoc[page - 1]);
    }

    // 4. 정렬된 데이터로 'users' 컬렉션의 모든 문서 가져오기
  ...
  // [6] 페이지네이션 초기화 함수
  const initPagenation = async () => {
    // 1. 전체 문서 가져오기
    const allDocs = await getDocs(query(collection(db, "users"), orderBy(sortField, sortOrder)));
    // 2. 전체 문서 데이터 구하기
